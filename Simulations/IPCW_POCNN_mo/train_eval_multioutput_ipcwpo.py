...
def sim_event_times_case1(trainset, num_samples):   
    ...
    r('data_surv_pso <- data.frame("time_r"=time_r,"event_r"=event_r , "cov_0"=cov_8, "cov_1"=cov_9, "cov_2"=cov_10, "cov_3"=cov_11,"cov_4"=cov_12,"cov_5"=cov_13,"cov_6"=cov_14, "cov_7"=cov_15, "cov_8"=cov_16,"cov_9"=cov_17) ' )
...
class PSODataset_train(Dataset):
    ...
    def __init__(self, clinical_data, transform):
        self.images = dataset = torchvision.datasets.CIFAR10(root='C:/Users/pabgon/project_2/dataset_cifar', train=False,download=False, transform=transform)
        #self.images = images
    ...
    def __getitem__(self, idx):
        ...
        return image, po1,po2,po3, po4, ID
...
class Dataset_test(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return image, time, event, event_1, event_2, event_3, event_4,  ID
...
def train_eval_resnet_sim(dataset,clindata,lr, niter):
    ...
    for epoch in range(niter):
        ...
        for i, data in enumerate(trainloader, 0):
            input_batch, po1_batch, po2_batch, po3_batch, po4_batch, slide_id_batch = data          
            ...
            po1_batch, po2_batch, po3_batch, po4_batch = po1_batch.to(device), po2_batch.to(device), po3_batch.to(device), po4_batch.to(device)
            # zero the parameter gradients
            optimizer.zero_grad()
            output_batch1, output_batch2,output_batch3,output_batch4 = net(input_batch.float(), covariates_batch )            
        ...
        with torch.no_grad():
            for i,(input_batch, time_batch, event_batch, event_batch_1, event_batch_2, event_batch_3, event_batch_4,  slide_id_batch) in enumerate(testloader,0):           
                input_batch = input_batch.to(device)
                covariates_batch = np.concatenate([np.array(clindata['covariates'][clindata['covariates']['ID']== int(file)].iloc[:,1:]).astype(np.float32) for file in slide_id_batch])            
                covariates_batch = torch.from_numpy(covariates_batch).to(device)
                output_batch1, output_batch2,output_batch3,output_batch4 = net(input_batch.float(),covariates_batch)
...
def main():
    ...
    #auc1    
    auc1_json_path = os.path.join(model_dir,"auc1_ipcwpo_mlt")
    np.save(auc1_json_path,AUC1_matrix)   
    #auc2
    auc2_json_path = os.path.join(model_dir,"auc2_ipcwpo_mlt")
    np.save(auc2_json_path,AUC2_matrix)
    #auc3
    auc3_json_path = os.path.join(model_dir,"auc3_ipcwpo_mlt")
    np.save(auc3_json_path,AUC3_matrix)
    #auc4
    auc4_json_path = os.path.join(model_dir,"auc4_ipcwpo_mlt")
    np.save(auc4_json_path,AUC4_matrix)
    #auc5
    #auc5_json_path = os.path.join(model_dir,"auc5_ipcwpo_mlt")
    #np.save(auc5_json_path,AUC5_matrix)
    #training loss
    train_loss_path = os.path.join(model_dir,"train_loss_ipcwpo_mlt")
    np.save(train_loss_path,train_loss_matrix)
    #val loss
    val_loss_path = os.path.join(model_dir,"val_loss_ipcwpo_mlt")
    np.save(val_loss_path,val_loss_matrix)
        
    ###########
    # CASE 2 # 
    model_dir =  'case2_N1000'
    if not os.path.exists(model_dir):
        os.mkdir(model_dir)
    print(model_dir)
    AUC1_matrix = np.zeros((25,num_sim))
