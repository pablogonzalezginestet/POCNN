...
def train_resnet(config, checkpoint_dir=None, data_dir=None):
...
if __name__ == "__main__":
    ...
    result = tune.run(
        ...
        name="pso_resnet18_balance_data",
        scheduler= scheduler,
    ...
    best_trained_model = Resnet18_Mtl(use_pretrained=False)
    device = "cpu"
    if torch.cuda.is_available():
        device = "cuda"
        if gpus_per_trial > 1:
    ...
    auc_average, auc_quantile = test_accuracy(best_trained_model, args.data_dir_test ,device)
